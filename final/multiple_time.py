...
with open('/mnt/rds/redhen/gallina/home/wxx170/frameblends_pipeline/nomination_algorithms/multiple_time/time_frame_list') as f:
    lines = f.readlines()
    frame_list = []
...
def word(path):
    ...
    for filename in rec_list:
                ...
                for f in frame_list:
                    if f.lower() == line.split('|')[5].lower():
                        ...
                        if len(current_frame_list) > 1:  # list is NOT empty before adding 1
                            ...
                            FBL_label.append('|')
                            FBL_label.append('multiple_time')
                            FBL_label.append('|')
        ...
        f1 = open('/mnt/rds/redhen/gallina/home/wxx170/frameblends_pipeline/nomination_algorithms/multiple_time/multiple_time_output/trial_2/' + real_name[ :-3] + 'fbl', "w")
        for item in output:
            f1.write("%s" % item)

        f2 = open('/mnt/rds/redhen/gallina/home/wxx170/frameblends_pipeline/nomination_algorithms/multiple_time/multiple_time_eval/trial_2/' + real_name[:-3] + 'txt', "w")
        for item in eval:
            f2.write("%s" % item)
